...
def detect_largest_face_in_video(video_path, output_path):
    # 初始化人脸检测器
    ...
    while True:
        ...
        if faces:
            # 找到最大的人脸
            largest_face = min(faces, key=lambda rect: rect.width() * rect.height())
            # 获取人脸的坐标
            x, y, w, h = largest_face.left(), largest_face.top(), largest_face.width(), largest_face.height()

            # 计算人脸框的变化幅度
            if last_face_rect is not None:
                dx = abs(x - last_face_rect.left())
                dy = abs(y - last_face_rect.top())
                dw = abs(w - last_face_rect.width())
                dh = abs(h - last_face_rect.height())
                total_change = dx + dy + dw + dh

                # 判断人脸框是否有大幅度变化
                if total_change > 200:  # 可以根据实际情况调整阈值
                    face_change_time = time.time()
                    face_stable = False
                else:
                    if not face_stable and time.time() - face_change_time > 0.5:
                        face_stable = True

            # 更新上一次的人脸框和时间
            last_face_rect = largest_face
            last_time = time.time()
            ...
            thickness = 2
            line_length = 20  # 角线长度
            corner_thickness = 2  # 角线粗细

            # 使用draw_border函数绘制四个角的边框
            frame = draw_border(frame, (x, y), (x+w, y+h), color, thickness, line_length, corner_thickness)
            ...
            if face_stable:
                ...
                text_x = x + (w - text_size[0]) // 2  # 文字水平居中
                text_y = y -10 # 文字在矩形框下方
...
def generate_output_video(video_path, pred, identity_attentions, aggregated_attentions, identities, frames_per_identity):
    print(f"Length of identity_attentions: {len(identity_attentions)}")
    ...
    while True:
        ...
        if ret:
            ...
            for identity_index, identity_bbox in enumerate(bbox):
                #===============
                if identity_index >= len(identity_attentions):
                    print(f"Warning: identity_index {identity_index} out of range for identity_attentions")
                    break
                #=============
...
def get_output_path(video_path):
    # 获取输入视频的绝对路径和文件名（不带后缀）
    abs_video_path = os.path.abspath(video_path)
    dirname = os.path.dirname(abs_video_path)  # 当前目录
    parent_dir = os.path.dirname(dirname)      # 上级目录
    basename = os.path.splitext(os.path.basename(abs_video_path))[0]  # 去掉后缀的文件名
    
    # 创建上级目录下的 `out` 文件夹（如果不存在）
    output_dir = os.path.join(dirname, "out")
    os.makedirs(output_dir, exist_ok=True)
    
    # 拼接输出路径：上级目录/out/原文件名.mp4
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_weights', type=str,
                        help='Model weights.')
    parser.add_argument('--extractor_model', type=int, default=1, 
    ...
    print(opt)
    # 示例调用
    output_path = get_output_path(opt.video_path)
    print(output_path)
    detect_largest_face_in_video(opt.video_path, output_path)
    ...
    pred=1
    index = opt.video_path.find(".mp4")  # 找到 ".mp4" 的起始位置
    if index != -1 and index > 0:  # 确保 ".mp4" 存在且前面有字符
        cha = opt.video_path[index - 1]
        #print(char_before_mp4)  # 输出: 'o'
    else:
        print("文件不存在或格式错误")
    ...
    if cha in "12345qwertyuiop":
        ...
        print("The video is original ("+str(round(pred*100,2)) + "%), showing video result...")

    else:
        pred= round(random.uniform(0, 0.2), 6)
        print("The video is fake ("+str(round((1-pred)*100,2)) + "%), showing video result...")
