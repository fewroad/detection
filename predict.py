...
def detect_faces(video_path, detector_cls: Type[VideoFaceDetector], opt):
    ...
    for item in loader: 
        ...
        if not found_faces:
            return {"no_faces": True}  # 返回字典而不是集合
...
def generate_output_video(video_path, pred, identity_attentions, aggregated_attentions, identities, frames_per_identity):
    ...
    output = cv2.VideoWriter("/home/qaq/video/out/"+str(os.path.basename(video_path).replace(".mp4", ".avi")), cv2.VideoWriter_fourcc("X", "V", "I", "D"), fps, (int(width), int(height)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_attentions', default=True, action="store_true",
    ...
    print(bboxes_dict)
    if isinstance(bboxes_dict, dict) and bboxes_dict.get("no_faces", False):
        
        print("No faces detected in the video.")
        result = {
        "no faces detected": True,
        "prediction": 0
        }
    ...
    result = {
        "prediction": pred,
        "identity_attentions": identity_attentions,
